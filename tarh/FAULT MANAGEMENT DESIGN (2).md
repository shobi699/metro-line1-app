...
**نسخه:** 1.0 — سند طراحی و سناریو برای توسعه ماژول `faults`
...
3. **موتور گزارش‌گیری و تحلیل:** فالت‌های ماندگار، تکراری، گزارش باز-زمانی (Aging)، تحلیل هر قطار نسبت به هر نوع فالت، و KPIهای نگهداشت (MTTR / MTBF / Pareto).
4. **شناسنامه فنی قطار (Train Health Profile):** تاریخچه کامل هر قطار در یک نما.
...
2. **نوع خطا:** انتخاب دسته (آیکون‌دار) ← جستجو/انتخاب کد خطا ← نمایش «راهنمای اقدام فوری» اگر تعریف شده. اگر کد پیدا نشد: گزینه «سایر» با توضیح آزاد (بعداً سرشیفت کد صحیح را می‌گذارد).
...
## ۱۰) فازبندی اجرا (Roadmap)

| فاز | محدوده | خروجی |
| :--- | :--- | :--- |
| **فاز ۱ (هسته)** | مدل‌های Prisma + Migration + Seed، CRUD ناوگان و کاتالوگ، ثبت فالت + گردش‌کار کامل + لاگ، کارتابل‌های سه‌گانه (وب) | چرخه ثبت‌تا‌بستن قابل استفاده |
| **فاز ۲ (هوشمندی)** | تشخیص تکرار، SLA + جاب نقض، auto-assign، نوتیفیکیشن SSE، ویزارد موبایل + آفلاین | گردش‌کار خودکار و بلادرنگ |
| **فاز ۳ (تحلیل)** | ۶ گزارش + داشبورد KPI + شناسنامه قطار + خروجی اکسل | لایه مدیریتی کامل |
| **فاز ۴ (ارتقا)** | RCA ساختاریافته، اتصال RAG (دستیار AI روی تاریخچه فالت‌ها: «سابقه ترمز قطار ۱۱۰ چیست؟»)، پیش‌بینی خرابی | پلتفرم نگهداشت هوشمند |

---

## ۱۱) ملاحظات فنی و امنیتی
