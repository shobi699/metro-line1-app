...
import { validateTrainsExcel, commitTrainsImport } from '@/server/modules/faults/import-export'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (mode === 'validate') {
      ...
      const preview = await validateTrainsExcel(buffer)
    ...
    if (mode === 'commit') {
      ...
      const result = await commitTrainsImport(validRows, user.id)
